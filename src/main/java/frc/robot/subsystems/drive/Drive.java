 ...
 * Handles all drive-related functionality including odometry, path following, and velocity control.
 */
public class Drive extends SubsystemBase {
  /** Drive base radius calculated from track width */
  ...
  /** Lock for thread-safe odometry updates */
  static final Lock odometryLock = new ReentrantLock();

  /** Gyro IO interface */
  private final GyroIO gyroIO;

  /** Auto-logged gyro inputs */
  private GyroIOInputsAutoLogged gyroInputs = new GyroIOInputsAutoLogged();

  /** Swerve modules array: [Front Left, Front Right, Back Left, Back Right] */
  private final Module[] modules = new Module[4];

  /** Swerve drive kinematics for converting chassis speeds to module states */
  ...
  /** Odometry class for tracking robot pose using wheel encoders and gyro */
  ...
  /** Simulated rotation for simulation mode */
  private Rotation2d simRotation = new Rotation2d();

  /** Deadzone chooser for autonomous pathfinding */
  private DeadzoneChooser deadzoneChooser = new DeadzoneChooser("Deadzone");

  /** Dashboard boolean for enabling/disabling vision */
  ...
  public Drive(
    ...
    // Start odometry thread for high-frequency position updates
    SparkMaxOdometryThread.getInstance().start();

    // Load PathPlanner robot configuration
    try {
    ...
    // Set up pathfinding with custom pathfinder
  ...
  public void periodic() {
    ...
    // Update gyro angle based on connection status
    ...
    // Update odometry with current gyro and module positions
   ...
   * This should be called when the robot's heading reference needs to be reset.
   *
   * @return null (command compatibility)
   */
  ...
  /**
   * Creates a command to pathfind to a target pose with specified end velocity.
   *
   * @param targetPose The target pose to pathfind to
   * @param endVelocity The desired end velocity in m/s
   * @param timeBeforeTurn Unused parameter (kept for compatibility)
   * @return Command to pathfind to the target pose
   */
  public Command goToPose(Pose2d targetPose, double endVelocity, double timeBeforeTurn) {
    return AutoBuilder.pathfindToPose(targetPose, kPathConstraints, endVelocity);
  }

  /**
   * Creates a command to pathfind to a target pose with zero end velocity.
   *
   * @param targetPose The target pose to pathfind to
   * @return Command to pathfind to the target pose
   */
  public Command goToPose(Pose2d targetPose) {
    return AutoBuilder.pathfindToPose(targetPose, kPathConstraints, 0.0);
  }

  /**
   * Gets an autonomous routine by name.
   *
   * @param nameString The name of the autonomous routine
   * @return Command representing the autonomous routine
   */
  ...
  /**
   * Creates a command to pathfind to a path, then follow it.
   *
   * @param path The PathPlanner path to pathfind to and follow
   * @return Command to pathfind to and follow the path
   */
