...
public class Lookup {
  ...
  private static double getValueFromColumn(double distance, int column) {
    ...
    // if the distance is below the lowest table distance, return a mapped value based on the least
    // two table distances
    if (distance <= LookupTable[0][0]) {
      return map(
          ...
          LookupTable[0][0],
          LookupTable[1][0],
          LookupTable[0][column],
          LookupTable[1][column]); // return mapped value based on the least two distances

      // if the distance is above the max table distance, return a mapped value based on the
      // greatest two table distances
    } else if (distance > LookupTable[LookupTable.length - 1][0]) {
      return map(
          distance,
          LookupTable[LookupTable.length - 2][0],
          LookupTable[LookupTable.length - 1][0],
          LookupTable[LookupTable.length - 2][column],
          LookupTable[LookupTable.length - 1][column]);
      ...
      return map(
          distance,
          LookupTable[lowerIndex][0],
          LookupTable[lowerIndex + 1][0],
          LookupTable[lowerIndex][column],
          LookupTable[lowerIndex + 1][column]);
    }
  }
  ...
  public static double getAngle(double distance) {
    return Rotation2d.fromDegrees(getValueFromColumn(distance, 2))
        .getRadians(); // 1 is the index of angle
  }
}
